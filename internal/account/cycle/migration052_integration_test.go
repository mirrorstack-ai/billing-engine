...
func TestMigration052_AdmittedTaskGPUModelsHaveExactPriceRows(t *testing.T) {
...
func TestMigration052_UpDownUp_RoundTrips(t *testing.T) {
	pool := testutil.NewTestDB(t)
	ctx := context.Background()

	_, err := pool.Exec(ctx, migrationSQL(t, "052_g5g_task_gpu_price.down.sql"))
	...
	_, err = pool.Exec(ctx, migrationSQL(t, "052_g5g_task_gpu_price.up.sql"))
	require.NoError(t, err)
	require.NoError(t, pool.QueryRow(ctx,
		`SELECT count(*) FROM ms_billing.metric_model_prices
...
func TestMigration052_TaskGPUExactModelPriceWinsOverVersionAndCatalog(t *testing.T) {
	pool := testutil.NewTestDB(t)
	ctx := context.Background()
...
func TestMigration052_UnknownTaskGPUModelCannotBillThroughHistoricalOrCatalogFallback(t *testing.T) {
	pool := testutil.NewTestDB(t)
	ctx := context.Background()
...
func TestMigration052_RemovingG5GPriceMakesBillingFailClosed(t *testing.T) {
	pool := testutil.NewTestDB(t)
	ctx := context.Background()
	acct, app := seedAccount(t, pool), uuid.New()
	_, err := pool.Exec(ctx,
...
func TestMigration052_ZeroG5GPriceMakesBillingFailClosed(t *testing.T) {
	pool := testutil.NewTestDB(t)
	ctx := context.Background()
	acct, app := seedAccount(t, pool), uuid.New()
	_, err := pool.Exec(ctx,
...
func TestMigration052_ModelPriceSchemaRejectsNegativePrice(t *testing.T) {
	pool := testutil.NewTestDB(t)
