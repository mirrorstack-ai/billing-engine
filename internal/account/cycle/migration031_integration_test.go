...
func TestProrationSkipped_Integration_OneShotAndExcludedFromPending(t *testing.T) {
	...
	skipped := uuid.New()
	require.NoError(t, store.InsertAppMirror(ctx, skipped, acct, uuid.Nil, 0, 0, mustTime(t, "2026-01-01T08:00:00Z"), "", ""))
...
func TestProrationSkipped_Integration_RefusesToSkipAnAlreadyChargedApp(t *testing.T) {
	...
	acct := seedAccount(t, pool)
	appID := uuid.New()
	require.NoError(t, store.InsertAppMirror(ctx, appID, acct, uuid.Nil, 0, 0, mustTime(t, "2026-01-01T08:00:00Z"), "", ""))
