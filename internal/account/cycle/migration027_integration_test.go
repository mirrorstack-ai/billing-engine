...
func TestAppsMirror_Integration_GuardSemantics(t *testing.T) {
	...
	require.NoError(t, store.InsertAppMirror(ctx, appID, acct, uuid.Nil, 2, created, ""))
	require.NoError(t, store.InsertAppMirror(ctx, appID, acct, uuid.Nil, 9, created.AddDate(0, 0, 5), ""))
...
func TestAppsMirror_Integration_LiveRosterScan(t *testing.T) {
	...
	require.NoError(t, store.InsertAppMirror(ctx, live1, acct, uuid.Nil, 0, created, ""))
	require.NoError(t, store.InsertAppMirror(ctx, live2, acct, uuid.Nil, 6, created, ""))
	require.NoError(t, store.InsertAppMirror(ctx, dead, acct, uuid.Nil, 9, created, ""))
	require.NoError(t, store.MarkAppDeleted(ctx, dead))
	require.NoError(t, store.InsertAppMirror(ctx, uuid.New(), other, uuid.Nil, 3, created, "")) // another account's app
	...
	require.NoError(t, store.InsertAppMirror(ctx, late, acct, uuid.Nil, 4, mustTime(t, "2026-07-01T10:00:00Z"), ""))
	...
	inGrace := uuid.New()
	require.NoError(t, store.InsertAppMirror(ctx, inGrace, acct, uuid.Nil, 1, mustTime(t, "2026-06-29T00:00:00Z"), ""))
	apps, err = store.LiveAppsCreatedBefore(ctx, acct, newPeriodStart, usage.GraceDays)
	require.NoError(t, err)
