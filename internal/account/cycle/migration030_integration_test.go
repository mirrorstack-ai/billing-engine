...
func TestCreatedModuleCount_Integration_FrozenAcrossSetAppModuleCount(t *testing.T) {
	...
	appID := uuid.New()
	require.NoError(t, store.InsertAppMirror(ctx, appID, acct, uuid.Nil, 2, 0, mustTime(t, "2026-07-01T08:00:00Z"), "", ""))
...
func TestCreatedModuleCount_Integration_RetryKeepsFirstRegistrationsFrozenCount(t *testing.T) {
	...
	require.NoError(t, store.InsertAppMirror(ctx, appID, acct, uuid.Nil, 3, 0, created, "", ""))
	require.NoError(t, store.InsertAppMirror(ctx, appID, acct, uuid.Nil, 12, 0, created, "", "")) // retry, different count
