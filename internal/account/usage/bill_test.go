...
func TestGetAppBill_PerAppBaseIsFlatEvenWithManyModules(t *testing.T) {
	// Migration 030: module overage is ACCOUNT-WIDE POOLED, so the PER-APP base
...
func TestGetAppBill_MirroredAppShowsFlatBase(t *testing.T) {
	// Migration 030: a mirrored app's per-app base is the FLAT $20 regardless of
...
func TestGetAppBill_UnmirroredAppShowsFlatBase(t *testing.T) {
	// No mirror row (pre-backfill) → the FLAT per-app base. Migration 030 retired
	// the pre-030 usage-proxy overage: overage is account-wide pooled, never
...
func TestGetAppBill_SnapshotFreezesChargedPeriodBaseAfterSync(t *testing.T) {
	...
	// per-app base (migration 030 — overage is pooled, not per-app), unaffected
