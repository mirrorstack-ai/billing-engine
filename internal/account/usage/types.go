...
type GetAppBillResponse struct {
	...
	// account-wide pooled (migration 030) and surfaced on GetAccountBill's
...
type GetAccountBillResponse struct {
	...
	// period (migration 030): $3 × max(0, Σ live-app module_count −
	...
	// CURRENT pooled sum. Additive field (pre-030 consumers that ignore it read
