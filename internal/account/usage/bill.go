...
const (
	...
	// surcharge are ACCOUNT-WIDE POOLED (migration 030 — see AccountOverageMicros),
	...
	// bundles before the per-module surcharge kicks in (migration 030 — ONE pool
...
//     pooled (migration 030, surfaced on GetAccountBill). An app ABSENT from
...
func (s *Service) computeAppBill(ctx context.Context, accountID uuid.UUID, found bool, plan Plan, appID uuid.UUID, periodStart, periodEnd time.Time) (*appBillParts, error) {
	...
	// pooled (migration 030), so the per-app base no longer needs a
	...
	if snapped {
		...
		case mirrored:
			...
			// overage is now account-wide pooled, migration 030 — never folded
		...
		default:
			// No mirror row (pre-backfill): the flat per-app base. The pre-030
