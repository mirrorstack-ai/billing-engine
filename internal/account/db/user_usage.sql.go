...
const repointUserNullAccountEvents = `-- name: RepointUserNullAccountEvents :execrows
...
type RepointUserNullAccountEventsParams struct {
...
func (q *Queries) RepointUserNullAccountEvents(ctx context.Context, arg RepointUserNullAccountEventsParams) (int64, error) {
	result, err := q.db.Exec(ctx, repointUserNullAccountEvents, arg.AccountID, arg.WindowStart, arg.OwnerUserID)
	if err != nil {
		return 0, err
	}
...
const userAccountsWithUnsweptUsage = `-- name: UserAccountsWithUnsweptUsage :many
  ...
  AND EXISTS (
      ...
      WHERE e.account_id IS NULL
        ...
        AND COALESCE(e.billable_at, e.recorded_at) >= GREATEST(
            ...
            ($1::timestamptz) - interval '31 days'
        )
  )
ORDER BY a.id
`

type UserAccountsWithUnsweptUsageRow struct {
	ID          string `json:"id"`
	OwnerUserID string `json:"owner_user_id"`
}
...
func (q *Queries) UserAccountsWithUnsweptUsage(ctx context.Context, asOf time.Time) ([]UserAccountsWithUnsweptUsageRow, error) {
	rows, err := q.db.Query(ctx, userAccountsWithUnsweptUsage, asOf)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []UserAccountsWithUnsweptUsageRow{}
	for rows.Next() {
		var i UserAccountsWithUnsweptUsageRow
...
const userUnbilledBacklogMicros = `-- name: UserUnbilledBacklogMicros :one
WITH base_events AS (
    SELECT
...
type UserUnbilledBacklogMicrosParams struct {
	OwnerUserID string    `json:"owner_user_id"`
	WindowStart time.Time `json:"window_start"`
}
...
// charged to anyone.
func (q *Queries) UserUnbilledBacklogMicros(ctx context.Context, arg UserUnbilledBacklogMicrosParams) (pgtype.Numeric, error) {
	row := q.db.QueryRow(ctx, userUnbilledBacklogMicros, arg.OwnerUserID, arg.WindowStart)
