...
WHERE link.customer_org_id = $1::uuid
  AND link.distributor_org_id = $2::uuid
`
...
func (q *Queries) GetDistributorCustomerAccount(ctx context.Context, arg GetDistributorCustomerAccountParams) (string, error) {
	row := q.db.QueryRow(ctx, getDistributorCustomerAccount, arg.CustomerOrgID, arg.DistributorOrgID)
...
WHERE link.distributor_org_id = $1::uuid
...
func (q *Queries) LockDistributorCustomerAccountForMutation(ctx context.Context, arg LockDistributorCustomerAccountForMutationParams) (string, error) {
	row := q.db.QueryRow(ctx, lockDistributorCustomerAccountForMutation, arg.CustomerOrgID, arg.DistributorOrgID)
	var account_id string
	err := row.Scan(&account_id)
	return account_id, err
}
