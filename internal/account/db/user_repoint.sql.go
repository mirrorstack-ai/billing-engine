...
WHERE account_id IS NULL
  AND app_id IN (
      ...
      WHERE account_id = $1::uuid AND owner_org_id IS NULL
  )
...
func (q *Queries) RepointUserNullAccountEvents(ctx context.Context, arg RepointUserNullAccountEventsParams) (int64, error) {
	result, err := q.db.Exec(ctx, repointUserNullAccountEvents, arg.AccountID, arg.WindowStart)
	if err != nil {
		return 0, err
	}
...
func (q *Queries) UserUnbilledBacklogMicros(ctx context.Context, arg UserUnbilledBacklogMicrosParams) (pgtype.Numeric, error) {
	row := q.db.QueryRow(ctx, userUnbilledBacklogMicros, arg.AccountID, arg.WindowStart)
	var backlog_micros pgtype.Numeric
	err := row.Scan(&backlog_micros)
	return backlog_micros, err
}

const usersWithUnsweptUsage = `-- name: UsersWithUnsweptUsage :many
SELECT a.id
...
// forever.
func (q *Queries) UsersWithUnsweptUsage(ctx context.Context) ([]string, error) {
	rows, err := q.db.Query(ctx, usersWithUnsweptUsage)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
